...
dependencies needed for running omnipath-metabo (`[tool.poetry.dependencies]` with the
...
developing omnipath-metabo using test-driven development. Please make sure that you
...
If you are brand new to omnipath-metabo or open-source development, we recommend
...
omnipath-metabo is hosted on GitHub, and to contribute, you will need to sign up for a
...
### Create a fork of omnipath-metabo

You will need your own fork of omnipath-metaboin order to eventually open a Pull
Request. Go to the omnipath-metabo project page and hit the Fork button. Please
...
```bash
git clone https://github.com/your-user-name/omnipath-metabo.git
cd omnipath-metabo
git remote add upstream https://github.com/omnipath-metabo/omnipath-metabo.git
git fetch upstream
```

This creates the directory `omnipath-metabo` and connects your repository to the
upstream (main project) *omnipath-metabo* repository. They have the same name, but
your local repository and fork are separate from the upstream repository.

### Creating a feature branch

Your local `main` branch should always reflect the current state of omnipath-metabo
repository. First ensure it's up-to-date with the main omnipath-metabo repository.
...
to create a branch called `my-new-feature-for-omnipath-metabo`

```bash
git checkout -b my-new-feature-for-omnipath-metabo
```

This changes your working branch from `main` to the
`my-new-feature-for-omnipath-metabo` branch. Keep any changes in this branch specific
to one bug or feature so it is clear what the branch brings to *omnipath-metabo*. You
...
When making changes, follow these omnipath-metabo-specific guidelines:
...
Now your code is on GitHub, but it is not yet a part of the omnipath-metabo project.
...
1. Write a descriptive title that includes prefixes. omnipath-metabo uses a convention for title prefixes, most commonly, `feat:` for features, `fix:` for bug fixes, and `refactor:` for refactoring.
...
branch with changes in the omnipath-metabo `main` branch, run:

```shell

    git checkout my-new-feature-for-omnipath-metabo
    git fetch upstream
...
```shell
    git push origin my-new-feature-for-omnipath-metabo
```
