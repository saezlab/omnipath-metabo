...
Then let's create a new STITCH COSMOS PKN build script by refering to scripts in Phase 1 using new_stitch. First I want to see outcome of new_stitch.interactions(). Save your investigation in STITCH_data_overview.md, and help me create a csv file with 5000 lines. We don't need subcellular location temporarily, because unlike transporter and receptor, the latter one needs to capture their trajectory. But we may add it in future if we want to know specific compartment where it happens. keep Gene__ and Metab__ prefixes. Please normalize orientation: Always put chemical as source, protein as target. And don't create reverse reactions. Put score as a parameter and set 700 as default, use final_score column. Put mode as another parameter and set binding as default. Don't consider stereospecificity at present. Set species as a parameter and 9606 as default. For mor column, keep the same as original table: activation = 1, inhibition = -1, unknown = 0.
