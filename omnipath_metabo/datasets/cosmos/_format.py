...
def format_pkn(
    ...
    is_non_pre_exp_transporter = df['_category'].eq('transporter') & ~is_pre_exp_mask
    reverse_mult = is_non_pre_exp_transporter.map({True: 4, False: 1})
    n_loc_gene = n_comps * n_genes
    n_final    = n_loc_gene * reverse_mult

    # Collect per-resource columns (ordered by first appearance)
    resources = list(dict.fromkeys(df['resource']))

    def _rc(res):
        m = df['resource'].eq(res)
        return (
            int(m.sum()),
            int(n_comps[m].sum()),
            int(n_loc_gene[m].sum()),
            int(n_final[m].sum()),
        )

    res_stats = {res: _rc(res) for res in resources}
    totals = (
        len(df),
        int(n_comps.sum()),
        int(n_loc_gene.sum()),
    ...
    col_w = 9
    label_w = 26
    header_res = ''.join(f'{r[:col_w]:>{col_w}}' for r in resources)
    sep = '-' * (label_w + 2 + col_w * (len(resources) + 1))

    def _row(label, idx):
        vals = ''.join(f'{res_stats[r][idx]:>{col_w},}' for r in resources)
        return f'  {label:<{label_w}}  {totals[idx]:>{col_w},}{vals}'

    lines = [
        '[COSMOS format] expansion steps:',
        f'  {"step":<{label_w}}  {"total":>{col_w}}{header_res}',
        f'  {sep}',
        _row('input edges',            0),
        _row('after location expand',  1),
        _row('after UniProt expand',   2),
        _row('after prefix/suffix ×4', 3),
        f'  {sep}',
        '  (pre-expanded: GEM/Recon3D ×1; other transporters: ×4)',
    ]
    _log.info('\n'.join(lines))
    # ------------------------------------------------------------------
